    ...
    try {
      ...
      if (response.ok) {
        ...
        if (response.status === 429) {
          ...
          rateLimitType = errorType || 'GENERAL';
            ...
            <form onsubmit={handleSubmit} class="space-y-6">
              ...
              <div>
                <label for="email" class="mb-2 block text-sm font-medium text-gray-700">
                  <i class="fas fa-envelope mr-2 text-blue-500"></i>이메일 주소<span class="text-red-500">*</span>
                </label>
                <input
                  id="email"
                  type="email"
                  ...
                  required
                  class="appearance-none relative block w-full px-3 py-2 border {emailField.error ? 'border-red-300' : 'border-gray-300'} placeholder-gray-500 text-gray-900 rounded-md focus:outline-none focus:ring-blue-500 focus:border-blue-500 focus:z-10 sm:text-sm"
                          ...
                          <div class="flex items-center">
                            ...
                            <span>
                              {#if cooldownSeconds >= 3600}
                                {Math.floor(cooldownSeconds / 3600)}시간 {Math.floor((cooldownSeconds % 3600) / 60)}분 후 재시도 가능
                              {:else if cooldownSeconds >= 60}
                                {Math.floor(cooldownSeconds / 60)}분 {cooldownSeconds % 60}초 후 재시도 가능
                              {:else}
              ...
              <div class="space-y-4">
                ...
                >
                  ...
                  {#if rateLimited && cooldownSeconds > 0}
                    {#if cooldownSeconds >= 3600}
                      {Math.floor(cooldownSeconds / 3600)}시간 {Math.floor((cooldownSeconds % 3600) / 60)}분 후 재시도
                    {:else if cooldownSeconds >= 60}
