...
{#if fullscreen}
	<div class="fixed inset-0 z-50 flex items-center justify-center bg-white bg-opacity-75">
		<div class="flex flex-col items-center space-y-4">
			{#if variant === 'spinner'}
				<div
					class="animate-spin rounded-full border-4 border-gray-200 border-t-blue-600 {sizeClasses[size].spinner}"
				></div>
...
{:else}
	{#if variant === 'spinner'}
		<div class="flex items-center justify-center space-x-2 {className}">
			<div
				class="animate-spin rounded-full border-2 border-gray-300 border-t-blue-600 {sizeClasses[size].spinner}"
			></div>
			{#if text}
