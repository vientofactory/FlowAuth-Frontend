...
{#if type === 'stats'}
	...
	<div class="mb-8 grid grid-cols-1 gap-6 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 {className}">
		{#each Array(count || 5) as _, i}
			<div class="overflow-hidden rounded-xl bg-gradient-to-br from-gray-200 to-gray-300 p-6 shadow-lg">
...
{:else if type === 'insights'}
	<!-- 인사이트 카드 스켈레톤 -->
	<div class="overflow-hidden rounded-xl border-l-4 border-l-gray-300 bg-gradient-to-r from-gray-50 to-gray-100 p-6 shadow-sm ring-1 ring-gray-200 {className}">
...
{:else if type === 'chart'}
	<!-- 실제 차트 카드와 일치하는 구조 -->
	<div class="overflow-hidden rounded-xl bg-white shadow-lg transition-shadow duration-300 hover:shadow-lg {className}">
		<div class="p-6">
			...
			<div class="h-80 animate-pulse rounded-lg bg-gray-50">
				<div class="flex h-full items-end justify-center space-x-1 p-4">
					{#each Array(12) as _, i}
						<div 
							class="w-6 animate-pulse bg-gray-200 rounded-t" 
...
{:else if type === 'table'}
	...
	<div class="relative overflow-hidden rounded-xl bg-gradient-to-r from-slate-50 to-gray-50 p-6 shadow-sm ring-1 ring-gray-100 {className}">
		<div class="relative">
			...
			<div class="space-y-4">
				{#each Array(count || 3) as _, i}
					<div class="group relative overflow-hidden rounded-lg border border-gray-100 bg-white/60 p-4 backdrop-blur-sm">
						<div class="flex flex-col space-y-4 sm:flex-row sm:items-start sm:justify-between sm:space-y-0">
							<div class="flex-1">
								<!-- 제목과 뱃지 -->
								<div class="mb-3 flex flex-col space-y-2 sm:flex-row sm:items-center sm:space-y-0 sm:space-x-3">
...
{:else if type === 'activity'}
	...
	<div class="space-y-4 {className}">
		{#each Array(count || 5) as _, i}
			<div class="group relative overflow-hidden rounded-xl border border-gray-100 bg-white/60 p-4 backdrop-blur-sm transition-all duration-200 hover:bg-white/80">
				<div class="flex items-start space-x-4">
					<div class="flex-shrink-0">
						<div class="h-10 w-10 animate-pulse rounded-full bg-gradient-to-br from-gray-200 to-gray-300"></div>
					</div>
					<div class="min-w-0 flex-1">
